...
int main(int argc, char **argv)
{
    ...
    if (file_system.node->magic != KRFFS_File_System_Magic) {
      fprintf(stderr, "Not a KRFFS file system.\n");
    ...
    if (file_system.node->type != KRFFS_Root_Node) {
      fprintf(stderr, "Not root node at the beginning of the file.\n");
    ...
    struct krffs_node *current_node = file_system.node;
    struct krffs_node *prev_node = current_node;
    do {

      if (prev_node > current_node) {
        fprintf(stderr, "Found a nonconsecutive link.\n");
        exit_status = KRFFS_Invalid_Link_Error;
        break;
      } 

      if (!(krffs_is_node_in_file_system(&file_system, current_node))) {
        fprintf(stderr, "Found a link leading outside the file system space.\n");
        exit_status = KRFFS_Out_of_Range_Node_Error;
        break;
      } 

      if (current_node->magic != KRFFS_File_System_Magic) {
        fprintf(stderr, "Found a node with an invalid signature.\n");
        exit_status = KRFFS_Invalid_Magic_Signature_Error;
        break;
      } 

      if (current_node->type != KRFFS_Root_Node &&
          current_node->type != KRFFS_Free_Node &&
          current_node->type != KRFFS_Reserved_Node) {
        fprintf(stderr, "Found a node of an unknown type.\n");
    ...
    } while ((current_node = krffs_get_next_node(&file_system, current_node)) != file_system.node);
